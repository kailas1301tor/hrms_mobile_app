...
class AdvanceRequestsListView extends ConsumerWidget {
  const AdvanceRequestsListView({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final list = ref.watch(
      adminRequestsProvider.select((s) => s.advanceRequests),
    );
    final loaderState = ref.watch(
      adminRequestsProvider.select((s) => s.advanceLoaderState),
    );
    final loadingMore = ref.watch(
      adminRequestsProvider.select((s) => s.advanceLoadingMore),
    );
    ...
    return CommonSwitchStateNoExpanded(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return SalaryAdvanceRequestCard(
            request: item,
            onApprove: () {
              ...
              showDialog(
                context: context,
                builder: (ctx) =>
                    ApproveAdvanceDialog(requestId: item.id ?? ''),
              );
            },
            onReject: () {
              notifier.clearActionControllers();
