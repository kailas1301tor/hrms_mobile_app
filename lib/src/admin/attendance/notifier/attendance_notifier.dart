...
class AttendanceNotifier extends _$AttendanceNotifier {
  AttendanceRepo get _repo => ref.read(attendanceRepositoryProvider);
  ...
  Future<void> getAttendance({
    ...
    if (isLoadMore) {
      ...
      state = state.copyWith(
        ...
        currentPage: 1,
        records: [], // Clear records to trigger shimmer
        summary: null,
      );
    }
    ...
    result.fold(
      ...
      (response) {
        ...
        final Map<String, AdminAttendanceResponse> newCache = Map.from(
          state.dataCache,
        );

        newCache[state.status] = AdminAttendanceResponse(
          data: mergedRecords,
